...
inline CSockAddrIn* CSockAddrIn::ConvertToSockAddrIn(const struct sockaddr* pstSockAddr)
{
    CSockAddrIn::m_oConvert.SetSockAddr(pstSockAddr);
    return &CSockAddrIn::m_oConvert;
}

inline CSockAddrIn* CSockAddrIn::ConvertToSockAddrIn(const char* pAddr, int iPort)
{
...
{
    return getsockopt(m_iFd, iLevel, iOptName, pOptVal, pOptLen) == 0;
}

inline bool CSocket::SetSendBuffLen(int iSendBuffLen)
{
    socklen_t uOptLen = sizeof(iSendBuffLen);
    return setsockopt(m_iFd, SOL_SOCKET, SO_SNDBUF, (void*)&iSendBuffLen, uOptLen) == 0;
}
...
inline int CSocket::GetSendBuffLen()
{
    int iSendBuffLen = 0;
    socklen_t uOptLen = sizeof(iSendBuffLen);
    return getsockopt(m_iFd, SOL_SOCKET, SO_SNDBUF, (void*)&iSendBuffLen, &uOptLen) == 0 ? iSendBuffLen : -1;
}

inline bool CSocket::SetRecvBuffLen(int iRecvBuffLen)
{
    socklen_t uOptLen = sizeof(iRecvBuffLen);
    return setsockopt(m_iFd, SOL_SOCKET, SO_RCVBUF, (void*)&iRecvBuffLen, uOptLen) == 0;
}
...
inline int CSocket::GetRecvBuffLen()
{
    int iRecvBuffLen = 0;
    socklen_t uOptLen = sizeof(iRecvBuffLen);
    return getsockopt(m_iFd, SOL_SOCKET, SO_RCVBUF, (void*)&iRecvBuffLen, &uOptLen) == 0 ? iRecvBuffLen : -1;
}
...
inline CSockAddrLl* CSockAddrLl::ConvertToSockAddrLl(const struct sockaddr* pstSockAddr)
{
    CSockAddrLl::m_oConvert.SetSockAddr(pstSockAddr);
    return &CSockAddrLl::m_oConvert;
}

inline CSockAddrLl* CSockAddrLl::ConvertToSockAddrLl(const char* pInterface, const char* pHardAddr)
{
